...
// does this work for an arbitrary penalty function (like the one i use now)? (maybe when using add it by 1/l (l be the max length of the sequences). 1/l comes from the fact that i normally get -log(mult(prob))=sigma(-log(prob)) and i want it to be more near 1 when l is bugger so -log(mult(prob)*exp(-1/l))=sigma(-log(prob))+-(-1/l). ALSO, note that if i add this 1/l when I wanna use the result (in main.java) then it is not very good. better is if i add 1/l in the recursive process of calculating it, so if i want to match tw subsequences, I add 1/l (l be the max of the length of these subsequences).
public class SequenceDistanceCalculator {
    public static class Matching<T> {
        public List<Map.Entry<T, T>> matching;
        public double score;
        public List<T> seq1;
        public List<T> seq2;

        public Matching(List<Map.Entry<T, T>> matching, double score, List<T> seq1, List<T> seq2) {
        ...
        public String toString() {
            ...
            for (Map.Entry<T, T> entry : matching)
                if (entry.getKey() == null)
                    res.append("_".repeat(entry.getValue().toString().length())).append("\t");
                else if (entry.getValue() == null)
                    res.append(entry.getKey()).append("\t");
                else
                    res.append(entry.getKey()).append(" ".repeat(Integer.max(0, entry.getValue().toString().length() - entry.getKey().toString().length()))).append("\t");
            res.append("\n");
            for (Map.Entry<T, T> entry : matching)
                if (entry.getValue() == null)
                    res.append("_".repeat(entry.getKey().toString().length())).append("\t");
                else if (entry.getKey() == null)
                    res.append(entry.getValue()).append("\t");
                else
                    res.append(entry.getValue()).append(" ".repeat(Integer.max(0, entry.getKey().toString().length() - entry.getValue().toString().length()))).append("\t");
            return res.toString();
        }
    }
    ...
    public static <T> Matching<T> calculate(List<T> seq1, List<T> seq2, BiFunction<T, T, Double> penalties) {
        ...
        for (int i = 0; i < scores.length; i++)
            for (int j = 0; j < scores[i].length; j++)
                scores[i][j] = -1;
        Map.Entry<T, T>[][] matchings = new Map.Entry[seq1.size() + 1][seq2.size() + 1];
        calculate(seq1, seq2, penalties, 0, 0, scores, matchings);
        List<Map.Entry<T, T>> result = new ArrayList<>();
        int i1 = 0, i2 = 0;
        while (matchings[i1][i2] != null) {
            result.add(matchings[i1][i2]);
            int addI1 = 0;
            if (matchings[i1][i2].getKey() != null)
                addI1 = 1;
            if (matchings[i1][i2].getValue() != null)
                i2++;
            i1 += addI1;
        }
    ...
    private static <T> void calculate(List<T> seq1, List<T> seq2, BiFunction<T, T, Double> penalties,
                                      int i1, int i2, double[][] scores, Map.Entry<T, T>[][] matchings) {
        if (scores[i1][i2] > -1)
            return;

        Map.Entry<T, T> matching = null;
        ...
        if (i1 < seq1.size() && i2 < seq2.size()) {
            ...
            if (penalty < Double.POSITIVE_INFINITY) {
                ...
                if (scores[i1 + 1][i2 + 1] + penalty < matchingScore) {
                    ...
                    matching = new AbstractMap.SimpleEntry<T, T>(seq1.get(i1), seq2.get(i2));
                }
            }
        }
        if (i2 < seq2.size()) {
            ...
            if (penalty < Double.POSITIVE_INFINITY) {
                ...
                if (scores[i1][i2 + 1] + penalty < matchingScore) {
                    ...
                    matching = new AbstractMap.SimpleEntry<T, T>(null, seq2.get(i2));
                }
            }
        }
        if (i1 < seq1.size()) {
            ...
            if (penalty < Double.POSITIVE_INFINITY) {
                ...
                if (scores[i1 + 1][i2] + penalty < matchingScore) {
                    ...
                    matching = new AbstractMap.SimpleEntry<T, T>(seq1.get(i1), null);
                }
            }
        }
